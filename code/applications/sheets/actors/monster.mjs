...
export default class RyuutamaMonsterSheet extends RyuutamaBaseActorSheet {
  ...
  #prepareTags() {
    ...
    level = _loc("RYUUTAMA.ACTOR.MONSTER.level", { level });
    ...
    if (this.document.system.details.category in ryuutama.config.monsterCategories) {
      ...
      tags.push({
        tag,
        tooltip: _loc("RYUUTAMA.ACTOR.MONSTER.category", { category: tag }),
      });
    }
    ...
    if (this.document.system.details.dragonica) {
      ...
      const tooltip = _loc("RYUUTAMA.ACTOR.MONSTER.dragonica", { number: tag });
    ...
    if (label) tags.push({
      tag: label,
      tooltip: _loc("RYUUTAMA.ACTOR.MONSTER.season", { season: label }),
    });
